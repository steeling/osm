...
func (s *fakeServer) ServeConfig(ctx context.Context, proxy *envoy.Proxy, config fakeConfig) error {
...
func TestControlLoop(t *testing.T) {
	...
	// p1 := envoy.NewProxy(envoy.KindSidecar, uuid.New(), identity.New("p1", "ns1"), nil, 1)
	// p2 := envoy.NewProxy(envoy.KindSidecar, uuid.New(), identity.New("p2", "ns2"), nil, 2)

	uuid1 := uuid.New()
	id1 := identity.New("p2", "ns2")
	cert1, err := certManager.IssueCertificate(envoy.NewXDSCertCNPrefix(uuid1, envoy.KindSidecar, id1), certificate.Internal)
	tassert.NoError(err)

	uuid2 := uuid.New()
	id2 := identity.New("p2", "ns2")
	cert2, err := certManager.IssueCertificate(envoy.NewXDSCertCNPrefix(uuid2, envoy.KindSidecar, id2), certificate.Internal)
	tassert.NoError(err)
