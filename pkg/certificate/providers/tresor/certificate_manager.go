...
func (cm *CertManager) issue(cn certificate.CommonName, validityPeriod time.Duration) (*certificate.Certificate, error) {
	...
	x509Root, err := certificate.DecodePEMCertificate(cm.ca.CertChain)
	if err != nil {
		// TODO(#3962): metric might not be scraped before process restart resulting from this error
	...
	rsaKeyRoot, err := certificate.DecodePEMPrivateKey(cm.ca.PrivateKey)
	if err != nil {
		// TODO(#3962): metric might not be scraped before process restart resulting from this error
	...
	cert := &certificate.Certificate{
		...
		IssuingCA:    pem.RootCertificate(cm.ca.CertChain),
...
func (cm *CertManager) IssueCertificate(cn certificate.CommonName, validityPeriod time.Duration) (*certificate.Certificate, error) {
	...
	log.Trace().Msgf("It took %+v to issue certificate with SerialNumber=%s", time.Since(start), cert.SerialNumber)
...
func (cm *CertManager) RotateCertificate(cn certificate.CommonName) (*certificate.Certificate, error) {
	...
	oldCert := oldObj.(*certificate.Certificate)
