	...
	Context("Test Hashi Vault functions", func() {
		...
		getCachedCertificateCNs := func() []certificate.CommonName {
			...
			cm.cache.Range(func(cnInterface interface{}, certInterface interface{}) bool {
				...
				commonNames = append(commonNames, cert.CommonName)
		...
		It("implements Certificate correctly", func() {
			...
			Expect(actual.CertChain).To(Equal([]byte(validCert.CertChain)))
			Expect(actual.Expiration).To(Equal(validCert.Expiration))
			Expect(actual.IssuingCA).To(Equal([]byte(issuingCA)))
		})
	})
})
