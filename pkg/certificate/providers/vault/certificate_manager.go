...
func (cm *CertManager) getIssuingCA(issue func(certificate.CommonName, time.Duration) (*certificate.Certificate, error)) ([]byte, certificate.SerialNumber, error) {
	...
	cm.ReleaseCertificate(cert.CommonName)

	return issuingCA, cert.SerialNumber, err
}
...
func (cm *CertManager) getFromCache(cn certificate.CommonName) *certificate.Certificate {
	if certificateInterface, exists := cm.cache.Load(cn); exists {
		...
		log.Trace().Msgf("Certificate found in cache SerialNumber=%s", cert.SerialNumber)
		if rotor.ShouldRotate(cert) {
			log.Trace().Msgf("Certificate found in cache but has expired SerialNumber=%s", cert.SerialNumber)
			return nil
		}
		return cert
	}
...
func (cm *CertManager) IssueCertificate(cn certificate.CommonName, validityPeriod time.Duration) (*certificate.Certificate, error) {
	...
	log.Trace().Msgf("Issued new certificate with SerialNumber=%s took %+v", cert.SerialNumber, time.Since(start))
...
func (cm *CertManager) RotateCertificate(cn certificate.CommonName) (*certificate.Certificate, error) {
	...
	log.Debug().Msgf("Rotated certificate (old SerialNumber=%s) with new SerialNumber=%s took %+v", oldCert.(*certificate.Certificate).SerialNumber, newCert.SerialNumber, time.Since(start))
